...
class ServidorWebSocket:
    def __init__(self):
        self.clientes_python = {}  # {id: {'socket': socket, ...}}
    ...
    async def manejar_dashboard(self, websocket):
        ...
        self.log(f"Dashboard conectado. Total: {len(self.clientes_web)}")
        try:
            ...
            self.log("Dashboard desconectado")
    ...
    async def procesar_comando_dashboard(self, data):
        ...
        if action == 'send-file':
            ...
            if cliente_id:
                self.log(f"Reenviando archivo '{filename}' a {target}...")
                await self.loop.run_in_executor(
                    self.executor, self.enviar_archivo_cliente, cliente_id, filename, filedata
                )
            else:
                self.log(f"Error archivo: Cliente {target} no encontrado")
            ...
            if cliente_id:
                ...
                self.log(f"Error: Cliente {target} no encontrado")
    ...
    def enviar_archivo_cliente(self, cliente_id, filename, filedata):
        ...
        try:
            ...
            self.log(f"Archivo '{filename}' enviado exitosamente al cliente.")
        except Exception as e:
            self.log(f"Error enviando archivo a cliente {cliente_id}: {str(e)}")
    ...
    def enviar_comando_cliente(self, cliente_id, comando):
        ...
        try:
            ...
            self.log(f"Error enviando a cliente {cliente_id}: {str(e)}")
    ...
    async def procesar_mensaje_cliente(self, cliente_id, mensaje):
        ...
        if tipo == 'info_sistema':
            ...
            self.log(f"Archivo recibido de {identificador}: {mensaje.get('nombre')}")
            await self.broadcast_web({'type': 'client-file', 'client': identificador, 'name': mensaje.get('nombre'), 'data': mensaje.get('contenido')})
        
        # NUEVO: PROCESAR SOLICITUD DE STREAM
        elif tipo == 'solicitar_stream':
            self.log(f"Solicitud de stream recibida de {identificador}")
            await self.broadcast_web({
                'type': 'stream-request',
                'client': dir_str, # ID para el select
                'name': identificador
            })
            ...
            msg = mensaje.get('mensaje', '')
            self.log(f"{identificador} - {msg}")
            await self.broadcast_web({
                'type': 'authorization-response',
                'client': identificador,
                'action': mensaje.get('accion'),
                'authorized': mensaje.get('autorizado'),
                'message': msg
            })
    ...
    def desconectar_cliente_python(self, cliente_id):
        if cliente_id in self.clientes_python:
            ...
            if self.loop and self.loop.is_running():
                asyncio.run_coroutine_threadsafe(
                    self.broadcast_web({'type': 'client-disconnected', 'client': dir_str}),
                    self.loop
                )
            self.actualizar_lista_clientes_sync()
